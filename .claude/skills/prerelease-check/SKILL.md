...
description: >-
  ...
  ana-docs and tech-docs sites scaffolded by the setup wizard, verifying
...
generator, a CLI (`tf-doc-vault` with a `setup` wizard plus subcommands), a
boilerplate/templates scaffold, plus Docker and Terraform. Because downstream
repos pin to the published npm package, the two things that hurt most on a bad
release are
...
(the dispatcher + flag/usage text), and `src/cli/setup.ts` (the setup wizard).

```bash
LAST=$(git describe --tags --abbrev=0 2>/dev/null)
git diff --stat ${LAST:+$LAST..}HEAD -- \
  package.json src/index.ts src/cli/tf-doc-vault.ts src/cli/setup.ts || \
...
- `bin` is still just `tf-doc-vault` (no separate scaffolder bin; `setup` is a
  dispatcher subcommand).
- `exports` still includes `.`, `./config`, `./sidebar`, `./theme`, the two
  `./theme/styles/*.css` entries, `./scripts/*`, the shared-config subpaths
  `./eslint`, `./prettier` and `./tsconfig`, the wildcards `./boilerplate/*`
  and `./templates/*`, `./infra/terraform`, the three explicit
  `./docker/<file>` paths, and `./package.json`.
- `src/index.ts` still exports `makeConfig`, `createTheme`, and the sidebar
  generators. Nothing under `src/` names a specific template (no `ana`, no
  `tech-docs`); the wizard reads that from `templates/*/_template.md` instead.
  This is a hard limit in AGENTS.md.
- Dispatcher subcommands unchanged: `setup`, `import-confluence`,
  `export-pdf`, `pdf`, `validate`, `normalize`, `fix`.
...
real Chromium with hydration/console-error assertions: `setup --template=ana-docs`
(dev + preview), `setup --template=tech-docs` preview, CLI subcommands
(`setup --help`, `import-confluence --help`, `print`, `ensure-lf`, `normalize`),
every subpath export, and the theme/dark-mode. Check the Playwright output for
...
ls -d "$SMOKE"/ana-parent/ana_test/docs/.vitepress/dist "$SMOKE"/tech-docs/tech-docs/docs/.vitepress/dist
```

The `tech-docs` path repeats itself because the fixture is a host repo at
`$SMOKE/tech-docs` and the wizard writes into a `tech-docs/` subfolder of it.
...
(`BrandHero`, `FeatureCards`) lives.

These are the sites and their **base paths** (the base is baked into each build,
so a site with a non-`/` base has to be opened at that path):

| Site       | Source to serve                                 | Base          |
| ---------- | ----------------------------------------------- | ------------- |
| playground | `playground/docs` (run `pnpm build:docs` first) | `/`           |
| ana        | `$SMOKE/ana-parent/ana_test/docs`               | `/`           |
| tech-docs  | `$SMOKE/tech-docs/tech-docs/docs`               | `/tech-docs/` |

The `ana-docs` template builds with `base: "/"` because its Docker image serves
the site at the domain root; `tests/smoke/ana-served-at-root.spec.ts` asserts
exactly that. Only `tech-docs` has a non-root base.
...
2. For each site: `preview_start` it → `preview_screenshot` →
   `preview_console_logs` (level `warn`, then `error`). playground and ana are
   built with `base: "/"`, so the page they open on is the right one. tech-docs
   404s at `/`; navigate to its base first with `preview_eval`
   (`window.location.assign('/tech-docs/')`).
...
- **Base paths are correct.** This is the high-value check: links that omit the
  ...
  ```js
  // ana (base "/"): feature cards must point at /v1/...
...
- **No literal markdown.** Component slot text (e.g. `BrandHero`'s subtitle) is
  ...
  (it's usually a Vue template or component issue, not a release blocker).
...
> `http://127.0.0.1:4173/tech-docs/`). No `.claude/launch.json` needed that way.
...
API/commands ........ unchanged ✓   (bin, exports, src/index.ts, dispatcher all identical; no template name in src/**)
Quality gate ........ lint / typecheck / format / build / unit (<N> passed, 0 skipped) ✓
Smoke ............... <N>/<N> passed ✓  (ana-docs dev+preview, tech-docs preview, CLI subcommands, exports, theme)
Browser visual ......
  • playground (/) .......... renders, console clean ✓
  • ana (/) ................. renders; feature-card hrefs = /v1/... ✓; console clean ✓
